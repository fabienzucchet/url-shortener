...
async def get_url_by_id(url_id: int, db: Session = Depends(get_db), tsdb=Depends(get_timeseries_client), start: int = -60, step: int = 5):
    ...
    enriched_url = EnrichedUrl(url=crud.get_url_by_id(
        db=db, url_id=url_id), stats=stats)
    return enriched_url
...
async def get_url_by_short_url(short_url: str, db: Session = Depends(get_db), tsdb=Depends(get_timeseries_client)):
