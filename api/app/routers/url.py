...
async def put_url(
    url: schemas.UrlBase, db: Session = Depends(get_db), user=Depends(get_current_user)
):
...
async def get_url_by_id(
    ...
    if url.owner_username != user["login"]:
        raise HTTPException(status_code=403, detail="You are not the owner of this URL")
    stats = []
    if url is not None:
...
async def delete_url(
    url_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)
):
    url = crud.get_url_by_id(db=db, url_id=url_id)
    if url is None:
